...
SELECT @@servername as [Server],
    ...
    'Next Start Date'= CONVERT(DATETIME, RTRIM(NULLIF(js.next_run_date, 0)) + ' '
        ...
        ' ','0'),3,0,':'),6,0,':')),
    CASE
       WHEN lastrun.run_duration > 235959
           THEN CAST((CAST(LEFT(CAST(lastrun.run_duration AS VARCHAR), LEN(CAST(lastrun.run_duration AS VARCHAR)) - 4) AS INT) / 24) AS VARCHAR) + '.' + RIGHT('00' + CAST(CAST(LEFT(CAST(lastrun.run_duration AS VARCHAR), LEN(CAST(lastrun.run_duration AS VARCHAR)) - 4) AS INT) % 24 AS VARCHAR), 2) + ':' + STUFF(CAST(RIGHT(CAST(lastrun.run_duration AS VARCHAR), 4) AS VARCHAR(6)), 3, 0, ':')
       ELSE STUFF(STUFF(RIGHT(REPLICATE('0', 6) + CAST(lastrun.run_duration AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
       END AS [LastRunDuration (d.HH:MM:SS)],
    CASE
       WHEN maxdur.run_duration > 235959
           THEN CAST((CAST(LEFT(CAST(maxdur.run_duration AS VARCHAR), LEN(CAST(maxdur.run_duration AS VARCHAR)) - 4) AS INT) / 24) AS VARCHAR) + '.' + RIGHT('00' + CAST(CAST(LEFT(CAST(maxdur.run_duration AS VARCHAR), LEN(CAST(maxdur.run_duration AS VARCHAR)) - 4) AS INT) % 24 AS VARCHAR), 2) + ':' + STUFF(CAST(RIGHT(CAST(maxdur.run_duration AS VARCHAR), 4) AS VARCHAR(6)), 3, 0, ':')
       ELSE STUFF(STUFF(RIGHT(REPLICATE('0', 6) + CAST(maxdur.run_duration AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
       END AS [MaxDuration (d.HH:MM:SS)],
...
LEFT OUTER JOIN dbo.syscategories sc
...
ORDER BY [Job Name],[Job Category]
